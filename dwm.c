...
#define ColForeground           0
#define ColBackground           1
#define ColBorder               2
...
enum { NetSupported, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation,
       NetWMName, NetWMState, NetWMCheck, NetWMFullscreen, NetActiveWindow,
...
drawbar(Monitor *m)
{
    ...
    if (showsystray && m == systraytomon(m)) {
      x = m->ww - sw - getsystraywidth();
		  drw_text(drw, x, 0, sw, bh, 0, stext, 0);
    }
	}
  ...
  x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0);
	...
	if ((w = m->ww - sw - x) > bh) {
      ...
      if (showsystray && m == systraytomon(m)) {
			  drw_text(drw, x, 0, w - getsystraywidth(), bh, lrpad / 2, m->sel->name, 0);
      } else {
...
updatesystray(void) {
	...
	if (!systray) {
        ...
        scheme[SchemeSel][ColBackground].pixel);
	...
	/* redraw background */
	XSetForeground(dpy, drw->gc, scheme[SchemeNorm][ColBackground].pixel);
